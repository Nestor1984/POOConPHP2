 ...
 * - No pueden instanciarse por que no tienen referencias
 * asociadas a objetos.
 * - No pueden definir propiedades.
 * - Los metodos de una interfaz deben ser publicos, solo
 * estar definidos y sin cuerpo.
 * - Las clases que implementen una interfaz deben utilizar 
 * todos los metodos de la misma.
 * - Diferentes interfaces no pueden tener nombres de metodos
 *  identicos si seran implementadas por una misma clase.
 */

 // interface: coleccion de metodos abstractos
 ...
 // Simulamos la herencia multiple
 class Postre implements Ingredientes, Receta {
    ...
    // Implementamos los metodos de las 2 interfaces
    public function establecer_ingredientes($lista){
        $this->ingredientes = explode(",", $lista); // explode('caracterDelimitador', $cadena): explode() es una funcion que rompe una cadena y convierte en varias posiciones de un arreglo, delimitado por un caracter delimitador
    }
    ...
    public function establecer_receta($pasos){
        $this->receta = explode('|', $pasos); // explode('caracterDelimitador', $cadena)
    }
